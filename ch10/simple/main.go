...
func getProfile(writer http.ResponseWriter, req *http.Request) {
	...
	tmp, _ = tmp.ParseFiles(
		path.Join(currentPath, "template/index.html"),
		path.Join(currentPath, "template/table.html"))
...
func login(writer http.ResponseWriter, req *http.Request) {
	currentPath, _ := os.Getwd()
	temp, _ := template.ParseFiles(path.Join(currentPath, "template/index.html"), path.Join(currentPath, "template/login.html"))
...
func song(writer http.ResponseWriter, req *http.Request) {
	...
	temp, err := template.ParseFiles(
		path.Join(currentPath, "template/index.html"),
		path.Join(currentPath, "template/song.html"))
	if err != nil {
		log.Println(err)
		return
	}
...
func progress(writer http.ResponseWriter, req *http.Request) {
	...
	proStatus.Year = y
	currentPath, _ := os.Getwd()
	temp, _ := template.ParseFiles(path.Join(currentPath, "template/index.html"), path.Join(currentPath, "template/progress.html"))
...
func home(writer http.ResponseWriter, req *http.Request) {
	...
	t, err := t.ParseFiles(
		path.Join(currentPath, "template/index.html"),
		path.Join(currentPath, "template/home.html"))
	if err != nil {
		fmt.Println(err)
		return
	}
...
Go is expressive, concise, clean, and efficient. Its concurrency mechanisms make it easy to write programs that get the most out of multicore and networked machines, while its novel type system enables flexible and modular program construction. Go compiles quickly to machine code yet has the convenience of garbage collection and the power of run-time reflection. It's a fast, statically typed, compiled language that feels like a dynamically typed, interpreted language.`,
	...
	t, err := t.ParseFiles(
		path.Join(currentPath, "template/index.html"),
		path.Join(currentPath, "template/passage.html"))
	if err != nil {
		fmt.Println(err)
		return
	}
